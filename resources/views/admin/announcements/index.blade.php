...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <form method="GET" action="{{ route('admin.announcements.index') }}" class="grid md:grid-cols-3 gap-4">
                ...
                @forelse($posts as $post)
                    <tr class="border-t">
                        ...
                        <td class="p-3 flex gap-2">
                            <a href="{{ route('admin.announcements.show', $post) }}" class="text-blue-600">Detail</a>
                            <a href="{{ route('admin.announcements.edit', $post) }}" class="text-yellow-600">Edit</a>

                            <form action="{{ route('admin.announcements.destroy', $post) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus berita ini?')">
                                @csrf
                                @method('DELETE')
